...
export async function generatePlayerCard(player) {
  ...
  const cx = W / 2, cy = 118, R = 62;
  const parts = player.imieNazwisko.split(' ');

  if (photoImg) {
    // Clip to circle and draw photo (show top portion — player face)
    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cy, R, 0, Math.PI * 2);
    ctx.clip();

    // Fit image: cover the circle, anchor top-center
  ...
  // Logo + branding
  if (logoImg) {
    ctx.drawImage(logoImg, W / 2 - 26, 422, 52, 52);
  }
