...
function parseMatchDate(s) {
  ...
  for (const t of tokens) {
    ...
    const key = Object.keys(POLISH_MONTH_NUM).find(k => t.startsWith(k));
    if (key !== undefined) month = POLISH_MONTH_NUM[key];
  }
...
export function computeTeamStats(mecze) {
  ...
  for (const m of ligaMecze) {
    ...
    if (!m.wszystkie_zdarzenia?.length) continue;

    // goals scored & conceded by minute + HT score
    let htGf = 0, htGa = 0;
    for (const z of m.wszystkie_zdarzenia) {
    ...
    // scored first
    const firstGoal = m.wszystkie_zdarzenia.find((z) => z.typ === 'gol' || z.typ === 'samobój');
