...
export function getHerb(slug) {
  const file = join(process.cwd(), "public", "herby", `${slug}.jpg`);
  return existsSync(file) ? `/herby/${slug}.jpg` : null;
}

function addMatch(opponents, opp, match, sezonLabel) {
  const oppSlug = slugify(opp);
  if (!opponents.has(oppSlug)) {
    opponents.set(oppSlug, { nazwa: opp, slug: oppSlug, herb: getHerb(oppSlug), mecze: [], sezony: new Set() });
  }
