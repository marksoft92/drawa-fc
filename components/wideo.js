...
function VideoCard({ film }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', aspectRatio: isShort ? '9/16' : '16/9', background: '#000', cursor: 'pointer' }}
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        {visibleVideos.length > 0 && (
          <div style={{ marginTop: 24, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
            {visibleVideos.map(f => <VideoCard key={f.id} film={f} />)}
          </div>
        )}
        ...
        {visibleShorts.length > 0 && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
              {visibleShorts.map(f => <VideoCard key={f.id} film={f} />)}
            </div>
          </>
        )}
